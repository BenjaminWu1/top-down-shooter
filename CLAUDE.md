...
`README.md` is out of date — it describes **30 levels in 6 boss tiers**, but the current code has **40 levels grouped into 8 boss tiers of 5 levels each**. The game offers a 5-strong assistant roster (the player picks 2 per run) and a per-character skill set on **X (sprint) / C (class) / F (heal) / B (blast)**, with a Tank-only **V** fast sword-knight summon (`vsummon`). **Space pauses/resumes.** The 3rd character is named **Benjamin Wu**. Trust this file and the code, not the README, for current scope.
...
Each character in `CHARACTERS` has a `skills:{x, c, f, b, v?}` map and a matching `skillLabels`. **X / F / B are universal** (all three classes share them); **C is the class-unique slot**; the **V slot is optional and Tank-only**. Every key dispatches through `triggerSkill(name, dx, dy, mag)` which returns `true` on successful fire so the per-key cooldown only charges on success. Cooldown maximums live in `player.skillCdMax` (`sprint:20`, `heal:20`, `blast:20`, `vsummon:25`, `wall:18`, `slowmo` via the `c:12` default, …).
...
- **B is `blast`** (label `BLAST`, 20 s cooldown): a small-radius (26) AoE burst centered on the player via the shared `explode()` primitive, normal damage (`3 × damageMult`, doubled by the DAMAGE powerup).
- **V is Tank-only `vsummon`** (label `BLADE+`, 25 s cooldown): the one-alive-at-a-time sword knight (the old universal `summon`), spawned with `attackRate /= 1.5` (~1.5× faster swings). This is now the **only** way to summon a `sword_ally` — X no longer summons.

`drawSkillHUD()` pushes chips in order X, C, F, B, then V (V only when `ch.skills.v` exists). The character-select card lists the same slots, laid out sequentially (`sy` accumulator) so optional rows never overlap; the card is 172 px tall to fit Benjamin Wu's five skills. Each key's `updatePlaying` dispatch is gated on its `charSkills.<k>` entry, so adding/removing a slot on a class is purely a data change in `CHARACTERS`.

The old M-slot was removed. Several skill handlers (`dash`, `blink`, `grenade`, `nuke`, `flameburst`, `summon`) remain in `triggerSkill` but are currently unreferenced — `summon` was X's old sword-summon (superseded by `sprint` on X / `vsummon` on V), and `flameburst` was the Tank's even-older B-key skill (its `player.flameburstTime` timer is still decayed in `updatePlaying` and `tickFlameburst(dt)` still exists, but no key fires it). Leave them for future characters.
...
- **3rd character renamed TANK → "BENJAMIN WU"** (`CHARACTERS.tank.name`); its `key` is still `'tank'` and all class logic keys off `'tank'`, so only the display name changed.
