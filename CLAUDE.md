...
There is also an **optional headless runtime smoke-test: `tools/headless_check.py`**. Unlike `validate.py` (static), it actually *executes* the game's JS in a V8 engine (`pip install py_mini_racer`) with the canvas/DOM/audio replaced by no-op stubs, then drives `update()`/`draw()` across all 40 levels, force-spawns every mid-boss + end boss, and directly calls the boss-ability helpers + `maybeElite` + every draw state. It catches runtime errors the in-game `try/catch` swallows (ReferenceError/TypeError, a renamed-but-still-called function, etc.) — but it can't see pixels, so visual/aesthetic correctness still needs a browser reload. It is **not** wired into CI (CI stays stdlib-only); run it locally after risky logic changes: `python tools/headless_check.py`.
